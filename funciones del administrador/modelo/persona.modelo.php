<?php
    ...
    class Persona{
        ...
        public function Guardar($nombres , $contenido , $precio , $acciones )

        $conexion = new Conexion();
        $stmt = $conexion->prepare("INSERT INTO 'rba'.'persona'
                                                    ('nombres',
                                                    'contenido',
                                                    'precio',
                                                    'acciones',)
...
if($stmt->execute()){
    ...
    return "Error: se ha generado un error al modificar la informacion";
        ...
        $conexion = new Conexion();
        $stmt = $conexion->prepare("DELETE FROM persona WHERE idPromcion = idPromocion ")
        $stmt->bindValue(":idPromocion", $idPromociones, PDO::PARAM_INT);
